 ...
 */
static void outOfRangeSignalHandler(DBusGProxy * proxy, const char *valueName,
		gpointer userData)
{
	g_print(PROGNAME ":out-of-range (%s)!\n", valueName);
	if (!strcmp(valueName, "value1"))
		g_print(PROGNAME
			":out-of-range Value 1 is outside threshold\n");
	else
		g_print(PROGNAME
			":out-of-range Value 2 is outside threshold\n");
}
				      ...
				      const char *valueName, gpointer userData)
{
	...
	if (!strcmp(valueName, "value1")) {
		...
		if (!error)
			...
			handleError("Failed to retrieve value1", error->message,
					FALSE);
	} else {
		gdouble v = 0.0;
		...
		if (!error)
			...
			handleError("Failed to retrieve value2", error->message,
					FALSE);
	}
...
int main(int argc, char **argv)
{
	...
	remoteValue = dbus_g_proxy_new_for_name(bus, VALUE_SERVICE_NAME,	/* name */
